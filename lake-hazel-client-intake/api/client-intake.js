...
export default async function handler(req,res){
  ...
  const clickupResponse=await fetch(`https://api.clickup.com/api/v2/list/${CLICKUP_LIST_ID}/task`,{method:'POST',headers:{Authorization:process.env.CLICKUP_API_TOKEN,'Content-Type':'application/json'},body:JSON.stringify({name:taskName,markdown_description:description,status:'intake'})});
  ...
  return res.status(200).json({ok:true,taskId:data.id,taskUrl:data.url,attachmentErrors});
}
